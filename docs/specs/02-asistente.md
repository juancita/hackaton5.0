...
- La clave es `(canal, user_id)`. El estado es `{modo, paso, origen_id, destino_id, prioridad, candidatos[], updated_at}`.
- El TTL es de 30 min sin actividad; pasado ese tiempo, el estado vuelve a `INICIO`.

### R2 · Máquina de estados
```
INICIO ──(1|guiada)──▶ PASO_ORIGEN ─▶ PASO_DESTINO ─▶ PASO_PRIORIDAD ─▶ RESULTADO
   │                        ▲ ambiguo: lista numerada, responde el # o el nombre
   └──(2|manual)──▶ MANUAL  (texto libre; pregunta solo lo que falta)
RESULTADO ─▶ "1 Otra ruta" → PASO_ORIGEN/MANUAL · "2 Alternativas" · "3 Reportar"
```
- **INICIO:** "¡Hola! … ¿Cómo quieres que te ayude? 1️⃣ Guiada (paso a paso) 2️⃣ Manual (escribe libre)". Si el primer mensaje ya trae una ruta completa ("de Meissen a Paraíso"), se pasa directo a manual y se responde con la ruta.
- **Paso 1:** "¿Desde dónde sales?" · **Paso 2:** "¿A dónde te diriges?" · **Paso 3:** "¿Qué prefieres? 1 Rápido · 2 Barato · 3 Menos transbordos". Cualquier respuesta no reconocida en el paso 3 se toma como `rapido`.
...
`menu` / `reiniciar` / `inicio` → INICIO · `guiada` · `manual` · `ayuda`.
...
- `InboundMessage {canal, user_id, texto, nombre?}`.
- `OutboundMessage {texto, texto_base, opciones_rapidas[{id,label}], paso, modo, plan?: TripPlan, reporte?: IncidentView}`. Siempre lleva toda la información.

Adaptadores:
| Canal | Endpoint | Identidad | Render |
|---|---|---|---|
| Web | `POST /chat/web` | header `X-Client-Id` | Devuelve el `OutboundMessage` completo |
| Telegram | `POST /webhooks/telegram` | `message.from.id` | `sendMessage` + `reply_markup.keyboard` con las opciones |
| WhatsApp | `GET/POST /webhooks/whatsapp` | `contacts[0].wa_id` | Texto con la lista numerada (Cloud API) |
